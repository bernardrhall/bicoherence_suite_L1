...
if findCL
	...
	if x <= overload
		...
		dlmwrite(strcat(opath,'/','clusters_overload_',GWCN,'_','.dat'),x);
		flag = 1;
	end
else
	GWCN1 = strrep(GWCN,'.','_d_'); % need to replace '.' with '_d_' (or other valid character) so that the .fig/.png filename will work
...
%GW_channel_n = strsplit(GW_channel,'.');
%GW_channel_n = GW_channel_n{1:(end-1)}
%if length(GW_channel_n > 1)
%	GW_channel_n = strjoin(GW_channel_n,'.')
%else
%	GW_channel_n = GW_channel_n{1};
%end
GW_channel_n = strrep(GW_channel_n,'_','\_');
title_string = strcat('Bicoherence of&',GW_channel_n,' (@ f_s =&',num2str(fs),'),\newline GPS:&',num2str(GPS_central),', Dur. =&', ...
time_window,' sec.');
title_string = strrep(title_string,'&',' ');
title_string = strrep(title_string,'+',' ');
title_string = textwrap(cellstr(title_string),40);
title(title_string)
set(gca,'YDir','normal');
set(gca,'FontSize',str2double(fontsize));
xlabel('f1 (Hz)'), ylabel('f2 (Hz)')
